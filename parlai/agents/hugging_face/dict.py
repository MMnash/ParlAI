...
try:
    from transformers import GPT2Tokenizer, T5TokenizerFast, LEDTokenizer
except ImportError:
    raise ImportError(
...
class LEDDictionaryAgent(HuggingFaceDictionaryAgent):
    def get_tokenizer(self, opt):
        return LEDTokenizer.from_pretrained(opt['led_model_arch'])
